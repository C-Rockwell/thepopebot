...
// Module-level storage for scheduled tasks (used by kill switch)
let _scheduledTasks = [];
...
function loadCrons() {
  ...
  for (const cronEntry of crons) {
    ...
    const task = cron.schedule(schedule, async () => {
      try {
        const result = await executeAction(cronEntry, { cwd: cronDir, source: 'cron', actionName: name });
...
function stopAllCrons() {
  for (const { task, name } of _scheduledTasks) {
    try {
      task.stop();
    } catch (err) {
      console.error(`[cron] Failed to stop "${name}":`, err.message);
    }
  }
  console.log(`[cron] Stopped ${_scheduledTasks.length} cron tasks`);
}

/**
 * Restart all crons (stop existing, reload fresh).
 */
...
/**
 * Get count of active cron tasks.
 * @returns {number}
 */
function getCronCount() {
  return _scheduledTasks.length;
}

export { loadCrons, startBuiltinCrons, stopAllCrons, restartCrons, getCronCount, getUpdateAvailable, setUpdateAvailable, getInstalledVersion, isPrerelease };
