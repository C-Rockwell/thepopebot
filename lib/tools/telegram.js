...
/**
 * Send a voice/audio message to a Telegram chat
 * @param {string} botToken - Bot token from @BotFather
 * @param {number|string} chatId - Chat ID to send voice to
 * @param {Buffer} audioBuffer - Audio data (OGG/Opus preferred)
 * @param {Object} [options] - Additional options
 * @param {string} [options.caption] - Optional caption
 * @returns {Promise<Object>} - Sent message
 */
async function sendVoice(botToken, chatId, audioBuffer, options = {}) {
  const b = getBot(botToken);
  const { InputFile } = await import('grammy');
  const file = new InputFile(audioBuffer, 'voice.ogg');
  return b.api.sendVoice(chatId, file, {
