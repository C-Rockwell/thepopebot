...
import { isSttEnabled, transcribe } from '../voice/stt.js';
...
class TelegramAdapter extends ChannelAdapter {
  ...
  async receive(request) {
    ...
    if (message.voice) {
      ...
      if (!isSttEnabled()) {
        await sendMessage(
          this.botToken,
          chatId,
          'Voice messages are not supported. Please set GROQ_API_KEY or OPENAI_API_KEY to enable transcription.'
        );
        return null;
      }
      try {
        const { buffer, filename } = await downloadFile(this.botToken, message.voice.file_id);
        text = await transcribe(buffer, filename);
      } catch (err) {
    ...
    if (message.audio && !text) {
      ...
      if (!isSttEnabled()) {
        await sendMessage(
          this.botToken,
          chatId,
          'Audio messages are not supported. Please set GROQ_API_KEY or OPENAI_API_KEY to enable transcription.'
        );
        return null;
      }
      try {
    ...
    return {
      ...
      text: text || '',
      attachments,
      metadata: { messageId: message.message_id, chatId, isVoiceMessage },
    };
  }
  ...
  async sendVoiceResponse(threadId, audioBuffer, metadata, options = {}) {
    await sendVoice(this.botToken, threadId, audioBuffer, options);
  }

  get supportsVoiceOutput() {
    const config = getVoiceConfig();
    return config.enabled && config.channels.telegram.ttsEnabled;
  }
}
