...
/**
 * Get paginated memory browser entries with optional search/filter.
 * @param {number} [page=1]
 * @param {object} [filters]
 * @param {string} [filters.query] - FTS5 search query
 * @param {string} [filters.sourceType] - Filter by source type
 * @returns {Promise<{ entries: object[], total: number, page: number, pageSize: number }>}
 */
export async function getMemoryBrowser(page = 1, filters = {}) {
  await requireAuth();
  try {
    const pageSize = 20;
    const offset = (page - 1) * pageSize;
    if (filters.query?.trim()) {
      ...
      const results = searchMemories(filters.query.trim(), { limit: pageSize });
      return { entries: results, total: results.length, page, pageSize };
    }
    const { getMemories, getMemoryCount } = await import('../db/memories.js');
    const entries = getMemories({ sourceType: filters.sourceType, limit: pageSize, offset });
    const total = getMemoryCount({ sourceType: filters.sourceType });
    return { entries, total, page, pageSize };
  } catch (err) {
    console.error('Failed to get memory browser:', err);
    return { entries: [], total: 0, page, pageSize: 20 };
  }
}

/**
 * Delete a single memory entry by ID (audited).
 * @param {string} memoryId
 * @returns {Promise<{ success: boolean }>}
 */
export async function deleteMemoryEntry(memoryId) {
  await requireAuth();
  try {
    const { deleteMemoryById } = await import('../memory/manager.js');
    deleteMemoryById(memoryId, 'dashboard');
    return { success: true };
  } catch (err) {
