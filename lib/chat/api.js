...
export async function POST(request) {
  ...
  for (const part of fileParts) {
    ...
    if (mediaType.startsWith('audio/')) {
      // Audio files → transcribe via STT and inline as text
      try {
        const { isSttEnabled, transcribe } = await import('../voice/stt.js');
        if (isSttEnabled()) {
          const base64Data = url.split(',')[1];
          const audioBuffer = Buffer.from(base64Data, 'base64');
          const ext = mediaType.split('/')[1] || 'webm';
          const transcribedText = await transcribe(audioBuffer, `voice.${ext}`);
          userText += (userText ? '\n' : '') + transcribedText;
        }
      } catch (e) {
