...
import { searchMemories } from '../memory/manager.js';
...
const searchMemoryTool = tool(
  async ({ query }) => {
    try {
      ...
      return results
        .map((m) => `[${m.summary || 'Memory'}] ${m.content}`)
        .join('\n---\n');
    } catch (err) {
  ...
  },
  {
    name: 'search_memory',
    description:
      'Search persistent memory for context from previous conversations, job results, and saved knowledge. Use this to recall past interactions, decisions, and facts.',
    schema: z.object({
      query: z
        .string()
...
export { createJobTool, getJobStatusTool, getSystemTechnicalSpecsTool, getPiSkillCreationGuideTool, searchMemoryTool };
