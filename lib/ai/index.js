...
async function chat(threadId, message, attachments = [], options = {}) {
  ...
  // Auto-capture conversation memory (fire-and-forget)
  captureConversationMemory(threadId, options).catch(() => {});
...
/**
 * Auto-capture a conversation summary as a memory (fire-and-forget).
 * Only triggers after enough exchanges to be meaningful.
 */
async function captureConversationMemory(threadId, options = {}) {
  try {
    const { summarizeConversation } = await import('../memory/summarize.js');
    const msgs = getMessagesByChatId(threadId);
    await summarizeConversation(msgs, {
      threadId,
    ...
    if (err.message !== 'Memory system is disabled') {
      console.error('[autoCapture] Failed:', err.message);
    }
  }
}
...
async function summarizeJob(results) {
  try {
    ...
    // Store job summary as a memory (fire-and-forget)
    try {
      const { storeJobSummary } = await import('../memory/summarize.js');
      storeJobSummary(text.trim(), {
        jobId: results.job_id || results.pr_url || 'unknown',
        trustLevel: 'user-indirect',
      }).catch(() => {});
    } catch {}
